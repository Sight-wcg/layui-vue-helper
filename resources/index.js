...
function creatIconDoc(iconlist) {
  let _html = '<ul class="doc-icon">'
  iconlist.forEach((icon, index) => {
    _html += `
    <li onclick="copy('${icon.class}')">
      <i class="layui-icon ${icon.class}"></i>
      <div class="doc-icon-name">${icon.name}</div>
      <div class="doc-icon-fontclass">${icon.class}</div>
    </li>`
  })
  _html += '</ul>';
...
document.body.innerHTML = creatIconDoc(layIconList);